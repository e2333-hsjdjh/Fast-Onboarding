...
5. 按内容优先策略生成针对岗位的 Markdown 简历初稿。
6. 输出内容质量报告，提示缺少的真实证据、关键词覆盖和量化结果。
7. 输出 ATS 检查报告，提示缺失关键词和基础风险。
8. 可接入 Word 模板 agent 导出 `.docx`。
...
- `user_experiences`: 用户长期维护的个人经历库，包括经历类型、标题、组织、行动、技能和量化结果。
...
AI 可以根据用户已经输入的内容自动补全经历库和岗位项目字段，也可以作为聊天机器人提出改进意见和追问问题。
